<?php
    ...
    if (isset($_GET['id'])) {
        ...
        if ($result->num_rows > 0) {
            ...
            if ($ketNoi == 'Connected') {
                $sqlUpdate = "UPDATE `smart_printing`.`mayin` SET TinhTrang = 'Disabled' WHERE ID = '$printerID'";
                $conn->query($sqlUpdate);
                $conn->close();
                setcookie('ThongBao', 'Tắt máy in thành công.', time() + 5);
        ...
        setcookie('ThongBao', 'Tắt máy in thất bại!', time() + 5);
        header("location: ../../../view/management/managementDefault.php");
    }
